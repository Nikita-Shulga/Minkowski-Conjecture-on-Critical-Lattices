...
# 1. Define constants
const p_start = 203407 // 100000 
const p_end = 203767 // 100000 
...
println("  p range: [$p_start, $p_end] (step $p_step), ascending")
println("  σ range: [1, σ_p] (step $sigma_step), ascending")
...
p_values_sorted = p_values_asc 

for i in 1:(length(p_values_sorted) - 1)
    p_lo = interval(p_values_sorted[i])   # Lower value
    p_hi = interval(p_values_sorted[i+1]) # Higher value
    ...
    if isempty_interval(tp_result) # FIX: Use isempty_interval
    ...
    sigma_values_sorted = sigma_values_asc
    ...
    println("    Sigma range: ", interval(sigma_start, sigma_end), " with ", length(sigma_values_sorted)-1, " ascending steps")
    println("-"^40)
    ...
    local is_checking_g = false 
    local is_checking_h = false 

    local p_interval_failed = false

    # --- 5. Inner loop over sigma (ascending) ---
    for j in 1:(length(sigma_values_sorted) - 1)
        s_lo = sigma_values_sorted[j]   # Lower value
        s_hi = sigma_values_sorted[j+1] # Higher value
        ...
        if isempty_interval(tau_interval_rect) # FIX: Use isempty_interval
        ...
        # FIX: Use diam() and inf/sup
        should_subdivide_if_fails = (inf(tau_interval_rect) * 2 < sup(tau_interval_rect)) ||
                                    (diam(tau_interval_rect) > tau_subdivision_threshold)
        ...
        if is_checking_g && !is_checking_h
            ...
            # FIX: Use inf()
            g_inequality_holds = inf(g_interval) > g_h_precision

            if !g_inequality_holds 
                if !first_g_fail_printed; println("!!! FIRST FAIL for G: P=$(p_interval), S=$(s_interval), τ[1]=$(tau_interval_rect) (g.lo <= $g_h_precision)"); first_g_fail_printed = true; end
                ...
                if should_subdivide_if_fails
                    ...
                    for (k, current_tau_interval) in enumerate(tau_intervals_to_check)
                        ...
                        # FIX: Use inf()
                        if !(inf(g_interval_sub) > g_h_precision)
                            println("!!! SUB-FAIL for G: P=$(p_interval), S=$(s_interval), τ[$k]=$(current_tau_interval) (g.lo <= $g_h_precision)")
                            all_sub_intervals_passed = false
                            break 
                        end
                    end
        ...
        # --- 5f. STAGE 3: H CHECK ( < -1e-9 ) ---
        if is_checking_h
    h_interval = calculate_h_interval(p_interval, s_interval, tau_interval_rect)
    
    # CRITICAL FIX: Handle empty intervals
    if isempty_interval(h_interval)
        println("      !!! INCONCLUSIVE: h_bound is empty (Possible singularity at H2).")
        p_interval_failed = true
        break 
    end
    ...
    h_inequality_holds = sup(h_interval) < -g_h_precision
            ...
            if !h_inequality_holds 
                if !first_h_fail_printed; println("!!! FIRST FAIL for H: P=$(p_interval), S=$(s_interval), τ[1]=$(tau_interval_rect) (h.hi >= -$g_h_precision)"); first_h_fail_printed = true; end
                ...
                if should_subdivide_if_fails
                    ...
                    for (k, current_tau_interval) in enumerate(tau_intervals_to_check)
                        ...
                        if !(sup(h_interval_sub) < -g_h_precision)
                            println("!!! SUB-FAIL for H: P=$(p_interval), S=$(s_interval), τ[$k]=$(current_tau_interval) (h.hi >= -$g_h_precision)")
                            all_sub_intervals_passed = false
                            break 
                        end
                    end
...
# FIX: Check isempty before trying to merge
if isempty(failed_p_intervals)
    println("SUCCESS: All p-intervals were processed successfully.")
    println("       No inconclusive intervals found.")
else
    # Only call merge_intervals if there are actually failures
